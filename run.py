...
def hyperparameters_grid_search(model_name, dataset):
    ...
    for lr in learning_rate_options:
        for activation_func in activation_function_name_options:
            print(f"Current hyperparameters: num_epochs=5, learning_rate={lr}, activation_function_name={activation_func}")
            _, test_accuracy = train_and_test(model_name, dataset, num_epochs=5, learning_rate=lr, activation_function_name=activation_func)
            print(f"Current accuracy for test images: {test_accuracy}%")
            
            if test_accuracy > best_test_accuracy:
                best_test_accuracy = test_accuracy
                best_hyperparameters["learning_rate"] = lr
                best_hyperparameters["activation_function_name"] = activation_func
    print(f"Best test accuracy: {best_test_accuracy}%")
    print("Best hyperparameters:", best_hyperparameters)
